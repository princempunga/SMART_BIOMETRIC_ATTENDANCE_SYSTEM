...
<div id=\"addModal\" class=\"modal-overlay fixed inset-0 bg-[#0F172A]/60 backdrop-blur-sm z-[100] flex items-center justify-center hidden\">
    ...
    foreach($fields as $k => $v) {
        if ($k == 'faculty_id') {
            ...
            $html .= "            <div>
                <label class=\"block text-xs font-bold text-slate-400 uppercase tracking-widest mb-2\">{$v}</label>
                <input type=\"text\" name=\"{$k}\" required class=\"w-full px-4 py-3 rounded-xl bg-slate-50 border-slate-100 focus:bg-white focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 transition-all outline-none font-medium\">
            </div>\n";
        }
    }
...
<div id=\"editModal\" class=\"modal-overlay fixed inset-0 bg-[#0F172A]/60 backdrop-blur-sm z-[100] flex items-center justify-center hidden\">
    ...
    foreach($fields as $k => $v) {
        if ($k == 'faculty_id') {
            ...
            $html .= "            <div>
                <label class=\"block text-xs font-bold text-slate-400 uppercase tracking-widest mb-2\">{$v}</label>
                <input type=\"text\" name=\"{$k}\" id=\"edit_{$k}_{$plural}\" required class=\"w-full px-4 py-3 rounded-xl bg-slate-50 border-slate-100 focus:bg-white focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 transition-all outline-none font-medium\">
            </div>\n";
        }
    }
...
";
    ...
    foreach($fields as $k => $v) {
        $html .= "        document.getElementById('edit_{$k}_{$plural}').value = item.{$k};\n";
    }
